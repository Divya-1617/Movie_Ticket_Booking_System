...
<section class="image-text-section">
	...
	<img src="../photos/1 photo" width="50%">
    </div>
    ...
    <div class="text-container" >
       ...
       <h1 style="color: white;">Movie Ticket and Food Counter</h1>
        <div class="form" style="background-color: rgb(240, 240, 240);">
			 ...
			 <form id="booking-form" method="post" action="library_display.php">
	  ...
	  <table width='80%' border="1" cellspacing="0" style="text-align:center; margin-left:10px;">
	    <tr>
								<td><div align="center" class="style6">Customer no.</div></td>
	    ...
	    <label for="movie">Selected Movie:</label>
            <select id="movie" name="movie">
                <option value="150">Fast And Furious 7</option>
...
function calculateTotal() {
    ...
    const ticketPrice = 500; // Price per movie ticket
    const popcornPrice = 150; // Price per popcorn
    const sodaPrice = 100; // Price per soda
...
$qry="INSERT INTO `lbooking`( `rid`,`no`,`popcorn`,`soda`,`seating`,`location`,`date`,`time`,`total`) VALUES(NULL,'$no','$popcorn','$soda','$seating','$location','$date','$time','$total')";
$run=mysqli_query($con,$qry);

if($run==true)
{
	echo '<script>alert("Ticket booked Successfully...")</script>';
   echo '<script>window.location.href = "library_payment.php";</script>';
}
else
{
	echo '<script>alert("Ticket not booked...")</script>';
   echo '<script>window.location.href = "library_display.php";</script>';
}
}
?>
